...
def main() -> None:
    ...
    for epoch in range(args.epochs):
        ...
        model.eval()
        val_losses = []
        intersections = {class_id: 0 for class_id in eval_classes}
        unions = {class_id: 0 for class_id in eval_classes}
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                ...
                for class_id in eval_classes:
                    ...
                    intersections[class_id] += int(((pred_c & mask_c) & valid).sum().item())
                    unions[class_id] += int(((pred_c | mask_c) & valid).sum().item())
        ...
        for class_id in eval_classes:
            if unions[class_id] == 0:
                iou_by_class[class_id] = float("nan")
            else:
                iou_by_class[class_id] = intersections[class_id] / unions[class_id]

        valid_ious = [v for v in iou_by_class.values() if v == v]  # filtra NaN
        mean_iou = sum(valid_ious) / max(len(valid_ious), 1)

        iou_text = ", ".join(
            f"IoU_c{class_id}={iou_by_class[class_id]:.4f}" if iou_by_class[class_id] == iou_by_class[class_id]
            else f"IoU_c{class_id}=nan"
            for class_id in eval_classes
        )
